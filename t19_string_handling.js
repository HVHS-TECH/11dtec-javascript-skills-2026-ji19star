...
****************************/
console.log("Running t9_introduction.js");
...
function getFormInput() {
    ...
    let usersName = NAME_FEILD.value;
    count(usersName)
    if (isNaN(usersName) == false) {
        OUTPUT.innerHTML += "<br>Not valid name! <br> Please use letters.<br>";
    } else {
...
function count(_name) {
    const NAME_FEILD = document.getElementById("nameField");
    let = _name;
    const count = _name.length;
    console.log(count + "letters")

      if (count < 3) {
        OUTPUT.innerHTML += "Name must be longer than 3<br> ";
...
function verseInput() {
    ...
    OUTPUT.innerHTML += verses[verse] + " Bottles of milk on the wall <br>";
...
function itemInput() {
    ...
    let item = SHOP_FIELD.value;
    console.log("list");
    shopArray.push(item);
    OUTPUT.innerHTML += "You added " + item + " to your list <br>";
    console.log(shopArray);

};

function showList() {
    OUTPUT.innerHTML += "your list: " + shopArray + "<br>";
};
